      ...
      <CardContent className="p-0">
        ...
        <ul className={listClassName}>
          {features.map((feature) => {
            ...
            const featureHighlightClassName = feature.isHighlighted
              ? "bg-linear-to-b from-transparent from-50% to-accent-orange to-50%"
              : "";

            const featureItemClassName = cn(
              "relative leading-normal flex items-start gap-2",
              featureTextClassName,
              featureHighlightClassName
            );

            return (
              <li key={feature.text} className={featureItemClassName}>
